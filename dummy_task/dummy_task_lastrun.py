...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/janaki/Dropbox/GeffenLab/Janaki/uncertainty code/dummy_task/dummy_task_lastrun.py',
...
volume_check_sound = sound.Sound('A', secs=-1, stereo=True, hamming=True,
...
for thisVolume_check_trial in volume_check_trials:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    volume_check_sound.setSound("assets/" + Name, hamming=True)
    ...
    while continueRoutine:
        ...
        if volume_check_sound.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            ...
            volume_check_sound.play()  # start the sound (it finishes automatically)
    ...
    volume_check_trials.addData('volume_check_sound.started', volume_check_sound.tStart)
    volume_check_trials.addData('volume_check_sound.stopped', volume_check_sound.tStop)
...
for thisHeadphone_check_trial in headphone_check_trials:
    ...
    expt += 1
    text_3.setText(headphone_check_msg)
    # keep track of which components have finished
