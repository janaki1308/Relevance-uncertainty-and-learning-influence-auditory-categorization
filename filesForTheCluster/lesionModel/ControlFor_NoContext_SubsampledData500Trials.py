...
def subsampleLongContext(trial_behaviour_full, corrans_full, trial_tones_full):
    idxOfSubsampledData = np.random.choice(len(trial_behaviour_full), size=500, replace=True)
...
def posterior_array(freq_input, n_tones, p_low, log_prior):
    ...
    prior_dist_mixed_high = prior_high.copy() 
    ...
    prior_dist_mixed_low = prior_low.copy() 
...
if __name__ == '__main__':
    ...
    os.chdir("/home/janakis/ControlResults500Trials/noBiasFromProlific/pback_lesionmodel/subsampledData/")
        
    """
    Subsampling trials and behaviour
    """    
    for iteration in np.arange(int(sys.argv[8]),int(sys.argv[9])):
        ...
        write_into_file(params=[sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],sys.argv[7]],fresult = new_file)  
