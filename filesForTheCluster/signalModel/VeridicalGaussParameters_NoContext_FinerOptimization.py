...
def subsampleData(trial_behaviour_full, corrans_full, trial_tones_full):
...
def posterior_array(freq_input, n_tones, p_back, p_low, log_prior):
    ...
    if n_tones == 3:
        prior_tones_low = np.expand_dims(prior_dist_mixed_low@np.transpose(prior_dist_mixed_low),axis=2)@np.transpose(prior_dist_mixed_low)
        #p(T1,T2..|L) 
        prior_tones_high = np.expand_dims(prior_dist_mixed_high@np.transpose(prior_dist_mixed_high),axis=2)@np.transpose(prior_dist_mixed_high)
        #p(T1,T2..|H) 
    elif n_tones == 1:
...
def MLE_fmin(params, fresultFile):
    ...
    sigma_sensory, prob_back, prob_low = params[0], params[1], params[2] # inputs are guesses at our parameters  
        
    _,_,LikelihoodLatentTonegivenHigh,LikelihoodLatentTonegivenLow,_,_ = posterior_array(log_freq_seq_array, len(trial_tones[0]), p_back=prob_back, p_low=prob_low,log_prior=[log_prior_low_mean,log_prior_high_mean,log_prior_sigma])
...
def write_into_file(params, fresult):
    ...
    minimum_nll = scipy.optimize.fmin(MLE_fmin, params, args=(fresult,), maxiter=300,maxfun=300,xtol=0.01,ftol=0.01)

    #fresult.write("%s\n" % [minimum_nll,MLE_fmin(params, fresult)])
    #fresult.flush()
...
if __name__ == '__main__':
    ...
    os.chdir("/home/janakis/results/noBiasFromProlific/veridicalGaussianParamters_finerOptimization/")
        
    """
    Subsampling trials and behaviour
    """    
    for iteration in np.arange(int(sys.argv[6]),int(sys.argv[7])):
        trial_tones, trial_behaviour = subsampleData(trial_behaviour_full=trialBehaviour,corrans_full=corrAns,trial_tones_full=trialTones)
        filename = sys.argv[2] + str(iteration) + ".txt"
        new_file = open(filename,"a+")   
        write_into_file(params=[sys.argv[3],sys.argv[4],sys.argv[5]],fresult = new_file)  
