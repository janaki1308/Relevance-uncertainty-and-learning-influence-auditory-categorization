...
def write_into_file(std_sensory, fresult):
    """
    New optimization algorithm: uses scipy.optimize.fmin. 
    Crude grid initially and then find minimum using the function.
    """
    
    guess_sensory_sigma = np.array([std_sensory]);
    guess_p_back = np.arange(0.05,1,0.1); guess_p_low = np.arange(0.2,0.41,0.05)

    # Constraining guesses of means of low and high distributions based on observed behaviour in figure shown above. 

    neg_ll_array = np.zeros((len(guess_sensory_sigma), len(guess_p_back), len(guess_p_low)))
    for ss in range(len(guess_sensory_sigma)):
        for pb in range(len(guess_p_back)):
            for pl in range(len(guess_p_low)):
                params = [guess_sensory_sigma[ss], guess_p_back[pb], guess_p_low[pl]]
                neg_ll_array[ss,pb,pl],_ = MLE(params) 
                fresult.write("%s\n" % neg_ll_array[ss,pb,pl])
                fresult.flush()
...
if __name__ == '__main__':
    ...
    os.chdir("/home/janakis/results/longContextHighFromProlific/veridicalGaussianParameters_gridSearch/")
    new_file = open(sys.argv[2],"a+")
         
    write_into_file(std_sensory=float(sys.argv[3]),
                    fresult = new_file)  
